...
GDP_DJIA |>
 ggplot() +
  ...
  geom_text(aes(x = GDP, y = DJIA, label = Quarter), size = 5)
```
...
The scatterplot's trend line shows there is a sightly positive correlation between DJIA and GDP. The different data points also generally trend up. The correlation value is 0.95, which shows that there is a strong positive correlation. 
