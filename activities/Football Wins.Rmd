...
This model seems to be generally pretty useful because the prediction only deviates by about 2 wins and that the variability is pretty high. Additionally, a majority of the model's variance is explained by the model. However, I do think it's important to still be skeptical of the model and ensure that we are not using its predictions as fact or concrete evidence. The model should be used as more of a guide. 
...
Based on the correlation matrix and the correlation values, it looks like points scored have the most impact on wins. The takeaways have the next largest impact, with yards gained having the least impact. All of the factors have a pretty significant impact on wins. 
...
You could use this model to help discuss what teams or players are the most successful. You could use their performance in past games as the data set and compare this with other players. 